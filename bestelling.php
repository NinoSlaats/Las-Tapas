...
function berekenRekening($t) {
    $info = pakketInfo($t['pakket']);
    $regels = [];
    $labels = ['volw' => 'Volwassene', 'sen' => 'Senior (65+)', 'kind' => 'Kind'];
    foreach ($labels as $sleutel => $label) {
        $n = intval($t[$sleutel] ?? 0);
        if ($n > 0) {
            $regels[] = ['omschrijving' => "{$n}× {$label}", 'bedrag' => round($n * $info['prijs'][$sleutel], 2)];
        }
    }
    $drank = floatval($t['drank_kosten'] ?? 0);
    if ($drank > 0) {
        $regels[] = ['omschrijving' => 'Drankjes (niet inbegrepen)', 'bedrag' => round($drank, 2)];
    }
    ...
    if ($verl > 0 && $personen > 0) {
        ...
        $regels[] = [
            'omschrijving' => "Verlenging {$minuten} min ({$personen} pers.)",
...
try {
    ...
    // Bezorgde bestellingen na 10 minuten opruimen
    q("UPDATE bestellingen SET bezorgd_op = NOW() WHERE status = 'bezorgd' AND bezorgd_op IS NULL");
    q("DELETE FROM bestellingen WHERE status = 'bezorgd' AND bezorgd_op < (NOW() - INTERVAL 10 MINUTE)");
    ...
    if ($methode === 'GET') {
        ...
        $bestellingen = [];
        if ($isPersoneel && $tafelParam === 0) {
            $rijen = alleRijen(q("SELECT * FROM bestellingen ORDER BY id DESC"));
        } elseif ($tafelParam > 0) {
            vereisTafelCode($tafelParam, $code);
    ...
    if ($actie === 'update_tafel_status') {
        ...
        if ($status === 'bezet') {
            ...
            if (!$huidig) {
                q("INSERT INTO tafels (tafel, status, gast_naam, gezelschap, pakket, volw, sen, kind,
                                       start_tijd, eind_tijd, verlengingen, drank_kosten, betaalmethode, geaccepteerd_door, laatste_activiteit)
                   VALUES (?, 'bezet', ?, ?, ?, ?, ?, ?, NULL, NULL, 0, 0, '', '', NOW())",
                  "isssiii", [$tafel, $gastNaam, $gezelschap, $pakket, $volw, $sen, $kind]);
            } elseif ($huidig['start_tijd'] === null) {
                // Nog niets besteld: gegevens (ook arrangement) mogen nog worden aangepast
                q("UPDATE tafels SET gast_naam=?, gezelschap=?, pakket=?, volw=?, sen=?, kind=?, laatste_activiteit=NOW() WHERE tafel=?",
                  "sssiiii", [$gastNaam, $gezelschap, $pakket, $volw, $sen, $kind, $tafel]);
            }
            stuur(["succes" => true]);
        }
        ...
        if ($status === 'betalen') {
            ...
            q("UPDATE tafels SET status='betalen', betaalmethode=?, geaccepteerd_door='', laatste_activiteit=NOW() WHERE tafel=?",
              "si", [$betaalmethode, $tafel]);
            stuur(["succes" => true]);
        }
        ...
        if ($status === 'vrij') {
            ...
            if ($huidig) {
                ...
                sluitTafel($huidig, $reden, $medewerkerNaam ?: 'Personeel', $betaalmethode);
            }
            stuur(["succes" => true]);
        }
    ...
    if ($actie === 'verleng') {
        ...
        vereisTafelCode($tafel, $code);
        $t = haalTafel($tafel);
        if (!$t || $t['status'] !== 'bezet') fout("Deze tafel is niet in gebruik.");
    ...
    if ($actie === 'bewerk_gerecht') {
        ...
        q("UPDATE bestellingen SET gerecht=?, bedrag=? WHERE id=?", "sdi", [$nieuwGerecht, $nieuwBedrag, $id]);
    ...
    if ($actie === '' || $actie === 'bestel') {
        ...
        $t = haalTafel($tafel);
        if (!$t) fout("Meld je eerst aan bij deze tafel (naam en arrangement kiezen).");
